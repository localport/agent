...
func enterRaw(f *os.File) (restore func(), err error) {
	...
	if _, _, e := syscall.Syscall(syscall.SYS_IOCTL, f.Fd(), syscall.TCGETS, uintptr(unsafe.Pointer(&orig))); e != 0 {
		return nil, e
	}
	mod := orig
	mod.Lflag &^= syscall.ECHO | syscall.ICANON | syscall.ECHONL | syscall.IEXTEN
	if _, _, e := syscall.Syscall(syscall.SYS_IOCTL, f.Fd(), syscall.TCSETS, uintptr(unsafe.Pointer(&mod))); e != 0 {
		return nil, e
	}
	return func() {
